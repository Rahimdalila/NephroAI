...
def init_db():
    ...
    # Migration douce : colonnes profil médecin
    profile_cols = [
        ("phone",        "TEXT"),
        ("wilaya",       "TEXT"),
        ("ville",        "TEXT"),
    ...
    conn3 = get_db()
    for col, coltype in profile_cols:
        try:
            conn3.execute(f"ALTER TABLE users ADD COLUMN {col} {coltype}")
            conn3.commit()
        except Exception:
            pass
    conn3.close()
...
def get_patients():
    ...
    cur.execute("""SELECT
        ...
        pr.smoking, pr.alcohol, pr.hypertension,
        pr.coronary_artery_disease, pr.cancer,
        pr.chronic_liver_disease, pr.diabetic_retinopathy,
        pr.nsaid, pr.statin, pr.metformin,
        pr.insulin, pr.dpp4_inhibitor,
...
def get_profile():
    ...
    cur.execute("""SELECT id, username, email, nom, prenom, role, created_at,
                          phone, wilaya, ville, etablissement, specialite,
                          fonction, photo_base64
...
def change_password():
    ...
    cur.execute("SELECT id FROM users WHERE id=? AND password=?",
                (user["id"], hash_pw(old_pw)))
    if not cur.fetchone():
        conn.close()
        return jsonify({"success": False, "error": "Ancien mot de passe incorrect"}), 401
    conn.execute("UPDATE users SET password=? WHERE id=?", (hash_pw(new_pw), user["id"]))
    conn.commit()
    conn.close()
    return jsonify({"success": True})
...
def get_account_detail(account_id):
    ...
    cur  = conn.cursor()
    cur.execute(
        "SELECT id, username, email, nom, prenom, created_at FROM users WHERE id=? AND role='patient'",
        (account_id,)
    )
    row = cur.fetchone()
    if not row:
...
def delete_account(account_id):
    ...
    if not cur.fetchone():
        ...
        return jsonify({"success": False, "error": "Compte patient introuvable"}), 404
    cur.execute(
        "DELETE FROM predictions WHERE patient_id IN (SELECT id FROM patients WHERE user_id=?)",
        (account_id,)
    )
    cur.execute("DELETE FROM patients WHERE user_id=?", (account_id,))
    cur.execute("DELETE FROM tokens   WHERE user_id=?", (account_id,))
